            ...
            if previewer is not None or fallback_previewer is not None or ltx_previewer is not None or tiny_vae is not None:
                try:
                    ...
                    if self.save_frame_mode == "Original (Default)":
                        # 原版默认：每步仅存首帧，与原节点完全一致
                        self.frames.append(pil_first)

                    elif self.save_frame_mode == "Last Frame Per Step":
                        # 每步尾帧：每步仅存该步最后一帧
                    ...
                    elif self.save_frame_mode in ("All Frames", "Half Frames"):
                        # 仅最后一步生效：保存该步全部帧 / 一半帧
                        if step == total_steps_ - 1:
                            # 统一转为 RGB
                            for i in range(len(pil_frames)):
                                if pil_frames[i].mode != "RGB":
                                    pil_frames[i] = pil_frames[i].convert("RGB")
                            if self.save_frame_mode == "Half Frames":
                                self.frames = pil_frames[::2]
                            else:
                                self.frames = pil_frames.copy()
                    # ========== 修改结束 ==========
